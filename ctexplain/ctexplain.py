...
available_analyses = [
    Analysis("summary", lambda x: summary.report(summary.analyze(x)),
             "summarizes build graph size and how trimming could help"),
    Analysis(
        "culprits", lambda x: print("this analysis not yet implemented"),
        "shows which flags unnecessarily fork configured targets. These\n" +
        "are conceptually mergeable."),
    Analysis(
        "forked_targets", lambda x: print("this analysis not yet implemented"),
    ...
    Analysis(
        "cloned_targets", lambda x: print("this analysis not yet implemented"),
...
def _render_analysis_help_text() -> str:
    ...
    return "\n\n".join(f'- "{name}": {analysis.description}' for name, analysis in analyses.items())
...
parser.add_argument("-b",
                    ...
                    default=[],
                    nargs='+',
                    help="""command-line invocation of the build to analyze. For example:
...
def main(args):
    if not args.build:
        exit("ctexplain: build efficiency measurement tool. Add --help " + "for usage.")
    elif len(args.build) > 1:
        exit("TODO(gregce): support multi-build shareability analysis")

    (labels, build_flags) = _get_build_flags(args.build[0])
